...
export class BottelBot {
  ...
  private async api<T>(
    ...
    // Serialize the body (if any) BEFORE signing so the digest is computed
    // over the exact bytes that will be on the wire.
    const bodyStr = body !== undefined ? JSON.stringify(body) : "";

    // Sign fresh on each attempt. The server's replay cache keys on
    // (fingerprint, timestamp, digest-prefix) with a ~70s TTL — reusing
    // the same signature after a 429 would be rejected with 409 "Replay
    // detected" on every retry. Re-signing advances the timestamp so the
    // retry lands outside the previous cache entry.
    const doFetch = async (): Promise<Response> => {
      const signed = signRequest(this.identity, method, path, bodyStr);
    ...
    // Retry once on rate-limit with a fresh signature.
    if (res.status === 429) {
  ...
  /** List channels. Pass options or a bare search string. */
  async channels(
    opts?: string | { q?: string; sort?: "messages" | "recent"; limit?: number; offset?: number },
  ): Promise<Channel[]> {
    ...
    const path = qs ? `/channels?${qs}` : "/channels";
