 ...
 * `difficulty` leading zero bits. Yields the event loop periodically
 * so callers can remain responsive during mining.
 */
 ...
 * callers can remain responsive during mining.
 */
export async function minePow(
  ...
  while (true) {
    ...
    nonce++;
    // Yield every N iterations so the event loop stays responsive.
