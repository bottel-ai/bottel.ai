...
import { colors, columns, formatInstalls, Cursor, Rating, InstallCount, VerifiedBadge, HelpFooter } from "../cli_app_theme.js";
...
export function Home() {
  ...
  // Decode flat index → section + item
  const decoded = useMemo(() => {
    let remaining = selectedIndex;
    for (const sec of SECTIONS) {
      const size = sectionSizes[sec];
      if (remaining < size) return { section: sec, itemIndex: remaining };
      remaining -= size;
    }
  ...
  return (
    <Box flexDirection="column" paddingX={1}>
      ...
      {SECTIONS.map((sec) => {
        ...
        const title = sec === "menu" ? "Navigation" : sec === "featured" ? "Featured" : sec === "trending" ? "Trending" : "Categories";
        ...
        return (
          <Box key={`section-${sec}`} flexDirection="column">
            ...
            {/* Section content — only show if active */}
            {isActive && (
              <Box flexDirection="column" paddingLeft={2} marginBottom={1}>
                {sec === "menu" && MENU_ITEMS.map((item, i) => (
                  <Box key={`menu-${item.value}`}>
                    <Cursor active={i === activeItemIndex} />
                    <Box width={18}>
                      <Text color={i === activeItemIndex ? colors.primary : undefined} bold={i === activeItemIndex}>{item.label}</Text>
                    </Box>
                    <Text dimColor={i !== activeItemIndex} color={i === activeItemIndex ? colors.primary : undefined}>{item.description}</Text>
                  </Box>
                ))}

                {sec === "featured" && featuredAgents.map((agent, i) => (
                  <Box key={`featured-${agent.id}`}>
                    <Cursor active={i === activeItemIndex} />
                    <Box width={columns.name}><Text color={i === activeItemIndex ? colors.primary : undefined} bold={i === activeItemIndex}>{agent.name}</Text></Box>
                ...
                {sec === "trending" && trendingAgents.map((agent, i) => (
                ...
                {sec === "categories" && categories.map((cat, i) => (
                  <Box key={`category-${cat.name}`}>
                    <Cursor active={i === activeItemIndex} />
                    <Text color={i === activeItemIndex ? colors.primary : undefined} bold={i === activeItemIndex}>{cat.icon} {cat.name}</Text>
