...
export function ChannelView() {
  ...
  // Solo view: only show messages from self + channel owner
  ...
  // null = loading (haven't checked server yet); true/false = known
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") { setActionMenuOpen(false); leaveTriggerRef.current?.focus(); }
    };
  ...
  //
  // Scroll-anchor invariant: the message the user is currently looking at
  // must stay at the same vertical position after older messages are
  // prepended — otherwise the viewport "jumps" to the new top and the
  // user loses their reading place.
  //
  // We can't use requestAnimationFrame for the anchor: by the time rAF
  // fires, React's state update may not have flushed to the DOM yet, so
  // el.scrollHeight still reads the OLD value and scrollTop gets set to
  // 0 (the difference is 0). `flushSync` forces React to render and
  // commit the DOM synchronously so our scrollHeight measurement is
  // correct right after.
  const mergeOlder = useCallback(() => {
    ...
    // Flush the state update synchronously so el.scrollHeight below
    // reflects the new content before we re-anchor.
    flushSync(() => {
    ...
    if (el) {
      ...
      // New scrollTop = old scrollTop + height of prepended content.
      // Keeps whatever was at the top of the viewport exactly where it was.
  ...
  // Scroll messages to bottom ONLY on initial load (null → array transition).
  // Bug history: the previous version ran on every `messages` reference change,
  // so prepending older messages via mergeOlder() kicked the user back to the
  // bottom. Track null-ness across renders and fire only on that transition.
  const prevMessagesWasNullRef = useRef(true);
  useEffect(() => {
    const wasNull = prevMessagesWasNullRef.current;
    prevMessagesWasNullRef.current = messages === null;
    if (wasNull && messages && messages.length > 0) {
      setTimeout(() => {
        scrollToBottom();
        window.scrollTo(0, 0);
      }, 50);
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      ...
      if (cancelled) return;
      if (!token) return; // no identity → no live connection
      const wsUrl = `${wsBase}/channels/${encodeURIComponent(name)}/ws?token=${encodeURIComponent(token)}`;
      try {
              ...
              <>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4">
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 shrink-0 flex-wrap">
                    ...
                    {/* Join button (sits in the same row as stats + kebab) */}
                    {loggedIn && joined === false && joinStatus !== "pending" && (
                      <button
                        type="button"
                        onClick={handleJoin}
                        disabled={joining}
                        className="text-xs font-mono font-medium px-3 py-1 rounded-md bg-accent text-black hover:opacity-90 transition-opacity disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
                      >
                        {joining ? "Joining…" : "Join"}
                      </button>
                    )}
                    {/* Channel actions menu (Leave / Cancel request) */}
                    {(() => {
                      ...
                      if (!canLeave || confirmLeave) return null;
                      return (
                        <div ref={actionMenuRef} className="relative">
                          <button
                            ...
                            className="w-6 h-6 inline-flex items-center justify-center rounded-md text-text-muted hover:text-text-primary hover:bg-bg-elevated transition-colors cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
                          >
                          ...
                          {actionMenuOpen && (
                            <div
                              role="menu"
                              aria-label="Channel actions"
                              className="absolute right-0 top-full mt-1 z-10 min-w-[180px] rounded-md border border-border bg-bg-base shadow-lg py-1"
                            >
                              <button
                                type="button"
                                role="menuitem"
                                onClick={() => { setActionMenuOpen(false); setConfirmLeave(true); }}
                                className="w-full text-left px-3 py-1.5 text-xs font-mono text-text-secondary hover:bg-bg-elevated hover:text-text-primary transition-colors cursor-pointer"
                              >
                                {isPending ? "Cancel join request" : "Leave channel"}
                              </button>
                            </div>
                          )}
                        </div>
                ...
                {loggedIn && (() => {
                  ...
                  // Join button moved into the right-hand row. Not-joined,
                  // not-pending state has nothing to render here.
                  if (!joined && !isPending) return null;

                  // Pending or joined (non-owner) → single button that toggles confirm
                  if (!canLeave) return null;

                  // Kebab action menu is rendered in the right-hand row.
                  // Inline slot only shows pending-approval status + confirm UI.
                  if (!confirmLeave) return isPending ? (
                    <span className="mt-1.5 text-xs font-mono text-accent/80">Pending approval…</span>
                  ) : null;
