...
export function ChatList() {
  ...
  useEffect(() => {
    if (!showNew || searchQuery.trim().length < 1) {
      setSearchResults([]);
      return;
    }
  ...
  const handleCreateChat = async (fingerprint: string) => {
    ...
    try {
      ...
      setCreateError(err?.message || "Failed to start direct message");
    } finally {
      setCreating(false);
    }
  };
  ...
  const handleDelete = async (chatId: string) => {
    ...
    try {
      ...
      setActionError(err?.message || "Failed to delete direct message");
    }
  ...
  const handleApprove = async (chatId: string) => {
    ...
    try {
      ...
      setActionError(err?.message || "Failed to approve direct message");
    }
    setApproving(null);
  };
  ...
  return (
    <div className="py-6 sm:py-8">
      <Container>
        ...
        <Breadcrumb crumbs={[{ label: "Direct Messages" }]} />
          ...
          <div className="flex items-center gap-3">
            {loggedIn && (
              <div role="group" aria-label="Filter direct messages" className="flex gap-1 text-xs font-mono font-medium">
                <button
                  onClick={() => { setFilter("chats"); setFilterAnnounce("Showing active direct messages"); }}
                ...
                <button
                  onClick={() => { setFilter("requests"); setFilterAnnounce("Showing direct message requests"); }}
          ...
          <div className="border border-border rounded-lg p-4 mb-4">
            <div className="flex flex-col gap-3">
              ...
              <p className="text-xs font-mono text-text-muted" role="status" aria-live="polite" aria-atomic="true">
                {creating ? "Starting direct message..." : ""}
              </p>
              {/* Alert for create errors */}
              {createError && (
                <p id="chat-create-error" className="text-xs font-mono text-error" role="alert">
                  {createError}
              ...
              {searchQuery.trim().length >= 1 && searchResults.length === 0 && !creating && (
        ...
        {!loggedIn && (
          <div className="py-10 text-center">
            <p className="text-text-muted text-sm font-mono">
              A{" "}
              <Link to="/profile" className="text-accent hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-1 focus-visible:ring-offset-bg-base rounded-sm">keypair</Link>
              {" "}is required to send direct messages.
            </p>
          </div>
        )}
        ...
        {loggedIn && (
          <>
            ...
            {/* Loading */}
            {loading && chats === null && (
              <div className="flex flex-col" aria-busy="true" aria-label="Loading direct messages">
            ...
            {!loading && filtered !== null && filtered.length === 0 && (
              <div className="py-10 text-center">
                <p className="text-text-muted text-sm font-mono">
                  {filter === "requests" ? "No pending requests." : "No direct messages yet."}
                </p>
              </div>
            )}
                      ...
                      <span className="sm:px-2 sm:text-right flex items-center sm:justify-end gap-2 shrink-0">
                        {canApprove && (
                          <button
                            ...
                            aria-label={`Approve direct message request from ${name}`}
                            ...
                            {isConfirmingDelete ? (
                              ...
                              >
                                <span id={`delete-confirm-label-${chat.id}`} className="sr-only">
                                  Delete direct message with {name}?
                                </span>
                                <button
                                  type="button"
                                  onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleDelete(chat.id); }}
                                  aria-label={`Yes, delete direct message with ${name}`}
                              ...
                              <button
                                ...
                                aria-label={`Delete direct message with ${name}`}
